  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 md:ml-64 overflow-y-auto bg-[--color-bg]">
        ...
        <div className="flex items-center gap-3 h-14 px-4 border-b border-[--color-border] bg-white md:hidden">
          <button
            onClick={() => setSidebarOpen(true)}
            className="flex items-center justify-center w-8 h-8 rounded-md text-[--color-text-secondary] hover:bg-[--color-bg] transition-colors"
            aria-label="Open menu"
          >
            <Menu size={20} />
          </button>
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-md bg-[--color-brand] flex items-center justify-center">
              <span className="text-white text-xs font-bold leading-none">L</span>
            </div>
            <span className="font-semibold text-sm text-[--color-text-primary]">LAT System</span>
          </div>
        </div>
